        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-6">
          ...
          <div class="w-full sm:w-auto sm:flex-1 sm:max-w-xl">
            <div class="relative">
              ...
              <input v-model="searchTerm" @input="searchJobs" type="text" placeholder="Search Pending Jobs"
        ...
        <table class="w-full">
          <thead>
            <tr class="border-b border-gray-100">
              ...
              <th class="px-6 py-4 text-left text-sm font-semibold text-gray-600">Submitted Date</th>
          ...
          <tbody>
            <tr v-for="job in paginatedJobs" :key="job.id"
              ...
              <td class="px-6 py-4 text-gray-600">{{ job.submittedDate }}</td>
              <td class="px-6 py-4">
                <div class="flex items-center justify-end gap-2">
                  ...
                  <button @click="approveJob(job.id)"
                    class="p-2 text-gray-600 hover:text-green-600 transition-colors group relative">
                    <span
                      class="absolute bottom-full mb-2 hidden group-hover:block bg-gray-900 text-white text-xs px-2 py-1 rounded whitespace-nowrap">
                      Approve Job
                    </span>
      ...
      <div class="block lg:hidden divide-y divide-gray-100">
        <div v-for="job in paginatedJobs" :key="job.id" class="p-4 hover:bg-gray-50">
          ...
          <div class="grid grid-cols-3 gap-3 pt-3 border-t border-gray-100">
            ...
            <button @click="approveJob(job.id)"
              class="text-green-600 font-medium hover:text-green-800 active:text-green-900 py-2">
              Approve
            </button>
            <button @click="rejectJob(job.id)"
